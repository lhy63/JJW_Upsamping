import os
import sys
sys.path.append('/home/lqx/PycharmProjects/Measure/venv/lib/python3.6/site-packages')
import glob
import time
...
def format_result(psnr, ssim, lpips):
    return f'psnr = {psnr:0.2f}\tssim = {ssim:0.3f}\tlpips = {lpips:0.3f}'
...
def measure_dirs(dirA, dirB, use_gpu, verbose=False):
    ...
    for pathA, pathB in zip(paths_A, paths_B):
        ...
        d = time.time() - t
        vprint(f"compare {pathA.split('/')[-1]} with {pathB.split('/')[-1]}:\t {format_result(**result)}\t time = {d:0.1f}")
    ...
    vprint(f"Final Result: {format_result(psnr, ssim, lpips)}\t time = {time.time() - t_init:0.1f}s")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-dirA', default='/home/lqx/src_4k', type=str)
    parser.add_argument('-dirB', default='/home/lqx/upscaled', type=str)
